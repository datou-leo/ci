...
var ssx3=["province","city","county"];
var opt0=["省份","城市","区县"];
function setup_area(def0,def1,def2){
	var i,ob,flag;
	for(i=0;i<ssx3.length-1;i++)document.getElementById(ssx3[i]).onchange=new Function("change_area("+(i+1)+")");
	change_area(0);
	if(typeof def0!='undefined'){
		flag=false;
		ob=document.getElementById(ssx3[0]);
		for(i=0;i<ob.options.length;i++){
			if(def0==ob.options[i].value){
				ob.options[i].selected=true;
				flag=true;
				break;
			}
		}
		if(!flag)ob.options[0].selected=true;
		change_area(1);
		if(typeof def1!='undefined'){
			flag=false;
			ob=document.getElementById(ssx3[1]);
			for(i=0;i<ob.options.length;i++){
				if(def1==ob.options[i].value){
					ob.options[i].selected=true;
					flag=true;
					break;
				}
			...
			if(ssx3.length>2){
				...
				if(typeof def2!='undefined'){
					flag=false;
					ob=document.getElementById(ssx3[2]);
					for(i=0;i<ob.options.length;i++){
						if(def2==ob.options[i].value){
							ob.options[i].selected=true;
							flag=true;
							break;
						}
...
function change_area(v){
	var str="0";
	for(i=0;i<v;i++)str+=("_"+(document.getElementById(ssx3[i]).selectedIndex-1));
	var ss=document.getElementById(ssx3[v]);
	with(ss){
		length=0;
		options[0]=new Option(opt0[v],opt0[v]);
		if(v&&document.getElementById(ssx3[v-1]).selectedIndex>0||!v){
			if(dsy.Exists(str)){
				ar=dsy.Items[str];
		...
		if(++v<ssx3.length)change_area(v);
	}
}